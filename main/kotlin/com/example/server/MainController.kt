...
class MainController(rpc: NodeRPCConnection) {
    ...
    @PostMapping(value = [ "sample" ], produces = [ APPLICATION_JSON_VALUE  ], headers = [ "Content-Type=application/json" ])
    fun sampleRest(@RequestBody sampleReq: SampleReqClass): ResponseEntity<SampleReqClass> {

        val iouValue = sampleReq.iouValue
        val otherPartyName = sampleReq.otherPartyName
        val otherPartyLoc = sampleReq.otherPartyName
        val otherPartyCur = sampleReq.otherPartyCur




        return ResponseEntity.status(HttpStatus.OK).body(sampleReq)
    }
